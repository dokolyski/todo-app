...
export const TodoStore = signalStore(
  ...
  withMethods((store, todoApiService = inject(TodoApiService)) => ({
    ...
    addTodo: (todo: Omit<Todo, 'id'>) => {
      patchState(
        store,
        addEntity({ ...todo, id: generateId(store.entities().length) }),
      );
    },
    loadTemperatureForLocation: rxMethod<Todo | undefined>(
      pipe(
        filter(Boolean),
        switchMap((todo) =>
          todoApiService.loadCoordinates({ location: todo.location }).pipe(
            switchMap((response) => {
              if (response.results?.[0]) {
                return todoApiService.loadTemperatureForCoordinates(
                  response.results?.[0],
                );
              }
  ...
  withComputed((store) => ({
    filteredTodos: computed(() =>
      store
        .entities()
        .filter((todo) =>
          isMatchingSearchTerm(store.searchTerm().trim().split(' '), todo, [
            'date',
            'location',
            'content',
          ]),
    ...
    upcomingTodosNumber: computed(
      () =>
        store.entities().filter((todo) => new Date(todo.date) > new Date())
          .length,
    ),
  })),
);
